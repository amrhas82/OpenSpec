...
class TestUpdateCommand:
    ...
    def test_update_creates_agents_md(
        ...
        assert "openspec/AGENTS.md" in captured.out
    ...
    def test_error_if_openspec_directory_missing(
        ...
        """Should throw error if openspec directory does not exist."""
        with pytest.raises(
            RuntimeError,
            match="No OpenSpec directory found. Run 'openspec init' first."
        ):
            update_command.execute(str(temp_dir))
    ...
    def test_agents_template_has_critical_rules(
        ...
        assert "openspec validate" in content
