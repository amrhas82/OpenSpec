...
class TestArchiveCommand:
    ...
    def setup_openspec_structure(self, temp_dir):
        ...
        openspec_dir = temp_dir / "openspec"
...
Then expected result happens"""
        ...
        main_spec_path = (
            temp_dir / "openspec" / "specs" / "test-capability" / "spec.md"
        )
...
### Requirement: Thumbnail Field"""
        ...
        main_spec_path = temp_dir / "openspec" / "specs" / "gift-card" / "spec.md"
        updated_content = main_spec_path.read_text()
...
Modified content."""
        ...
        main_spec_path = (
            temp_dir / "openspec" / "specs" / "new-capability" / "spec.md"
        )
...
- TO: `### Requirement: New Name`"""
        ...
        # Verify spec was NOT created
        main_spec_path = (
            temp_dir / "openspec" / "specs" / "another-capability" / "spec.md"
        )
        assert not main_spec_path.exists()
    ...
    def test_error_if_archive_already_exists(
        ...
        archive_path = (
            temp_dir / "openspec" / "changes" / "archive" / f"{date}-{change_name}"
        )
...
- **THEN** it is captured"""
    ...
    def test_header_normalization(
        ...
        main_spec_dir = temp_dir / "openspec" / "specs" / "alpha"
...
Updated details."""
    ...
    def test_operations_applied_in_order(
        ...
        main_spec_dir = temp_dir / "openspec" / "specs" / "beta"
        main_spec_dir.mkdir(parents=True)
...
content D"""
    ...
    def test_error_on_missing_requirements(
        ...
        main_spec_dir = temp_dir / "openspec" / "specs" / "gamma"
        main_spec_dir.mkdir(parents=True)
...
### Requirement: Another Missing"""
    ...
    def test_modified_must_reference_new_header_after_rename(
        ...
        # Main spec with Old
        main_spec_dir = temp_dir / "openspec" / "specs" / "delta"
        main_spec_dir.mkdir(parents=True)
...
new body"""
    ...
    def test_multiple_specs_atomic(
        ...
        # Existing main specs
        epsilon_main = temp_dir / "openspec" / "specs" / "epsilon" / "spec.md"
...
e1"""
        ...
        zeta_main = temp_dir / "openspec" / "specs" / "zeta" / "spec.md"
...
### Requirement: Missing"""
    ...
    def test_aggregated_totals_across_multiple_specs(
        ...
        # Existing main specs
        omega_main = temp_dir / "openspec" / "specs" / "omega" / "spec.md"
        omega_main.parent.mkdir(parents=True)
        omega_main.write_text(
            "# omega Specification\n\n## Purpose\nOmega purpose.\n\n## Requirements\n\n### Requirement: O1\no1"
        )

        psi_main = temp_dir / "openspec" / "specs" / "psi" / "spec.md"
    ...
    def test_error_when_openspec_directory_missing(
        ...
        """Should throw error when openspec directory does not exist."""
        # Remove openspec directory
        shutil.rmtree(temp_dir / "openspec")

        with pytest.raises(
            RuntimeError,
            match="No OpenSpec changes directory found. Run 'openspec init' first.",
        ):
    ...
    def test_interactive_change_selection(
        ...
        change1 = "feature-a"
        change2 = "feature-b"
        (temp_dir / "openspec" / "changes" / change1).mkdir(parents=True)
        (temp_dir / "openspec" / "changes" / change2).mkdir(parents=True)
        ...
        change_dirs = list((temp_dir / "openspec" / "changes").iterdir())
