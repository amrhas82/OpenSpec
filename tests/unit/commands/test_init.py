...
class TestInitCommand:
    ...
    def test_create_openspec_directory_structure(self, init_command, temp_dir):
        ...
        assert openspec_path.exists()
        assert (openspec_path / "specs").exists()
        assert (openspec_path / "changes").exists()
        assert (openspec_path / "changes" / "archive").exists()
    ...
    def test_create_agents_md_and_project_md(self, init_command, temp_dir):
        ...
        openspec_path = temp_dir / "openspec"
        assert (openspec_path / "AGENTS.md").exists()
        assert (openspec_path / "project.md").exists()

        agents_content = (openspec_path / "AGENTS.md").read_text()
        assert "OpenSpec Instructions" in agents_content

        project_content = (openspec_path / "project.md").read_text()
    ...
    def test_extend_mode_preserves_existing_template_files(self, init_command, temp_dir):
        ...
        agents_path = temp_dir / "openspec" / "AGENTS.md"
    ...
    def test_recreate_deleted_agents_md_in_extend_mode(self, init_command, temp_dir):
        """Should recreate deleted openspec/AGENTS.md in extend mode."""
        # First init
        init_command.execute(str(temp_dir))
    ...
    def test_recreate_deleted_project_md_in_extend_mode(self, init_command, temp_dir):
        """Should recreate deleted openspec/project.md in extend mode."""
        # First init
        init_command.execute(str(temp_dir))

        project_path = temp_dir / "openspec" / "project.md"
    ...
    def test_handle_non_existent_target_directory(self, init_command, temp_dir):
        ...
        openspec_path = new_dir / "openspec"
        assert openspec_path.exists()
